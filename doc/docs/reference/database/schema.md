...
- Database: `tabflow_platform`
- Application role: `tabflow_platform_app`

Tenant database (one pair per tenant):

- Database: `tabflow_<tenant-code>`
- Application role: `tabflow_<tenant-code>_app`

`<tenant-code>` matches the tenant-code baseline described in
[`../../explanation/concepts/tenant-lifecycle.md`](../../explanation/concepts/tenant-lifecycle.md):
lowercase `a-z`, `0-9`, and hyphen, 3 to 63 characters, no leading or
trailing hyphen. Hyphens are preserved in both the database and role
name; PostgreSQL quotes them automatically in DDL.

The application role has `LOGIN`, `CONNECT`, and table-level
privileges on its own database only. It MUST NOT have superuser or
cross-database privileges; cross-tenant joins are an architectural
violation ([AD-0001](../architecture/decisions.md#ad-0001-platform-and-tenant-remain-architecturally-separate)).

Backup, restore, and rotation procedures in
...
- Snapshot per context: `Migrations/Platform/PlatformDbContextModelSnapshot.cs`,
  `Migrations/Tenant/TenantDbContextModelSnapshot.cs`

The migrations project is a standalone class library
(`src/infra/postgres/TabFlow.Migrations.csproj`) that references
`TabFlow.Shared` and exposes one `IDesignTimeDbContextFactory<T>` per context
so that EF Core design-time tooling resolves connection strings without
booting either host. Tenant bootstrap applies the committed tenant migration
history during provisioning. There is no handwritten idempotent `ALTER`
fallback.
...
## C# Entity Naming Conventions

The C# entity classes (`TabFlow.Shared.Domain.Entities.*`) carry their
own naming discipline that occasionally diverges from what a contributor
might guess. Property names that have surprised newcomers are listed
explicitly so the same wrong assumption is not repeated:

| Entity | Property | Common Wrong Guess |
| --- | --- | --- |
| `CustomerSession` | `IsOpen` | `IsActive` |
| `QrToken` | `Value` | `Token` |
| `ProvisioningJobStep` | `StartedAt` | `CreatedAt` |
| `Category` | `IsValid` | `IsInvalid` |
| `MenuItem` | `Name` | `Label` |
| `CartItem` | `ItemId` | `MenuItemId` |

Factory method discipline:

- `CustomerSession.IssueTicket()` is the only correct way to mint a
  `CustomerAccessTicket`. **Do not** call `CustomerAccessTicket.Create`
  directly.
- `ProvisioningJob.AddStep(name)` is the only correct way to add a
  `ProvisioningJobStep`. **Do not** instantiate the step directly.
- `TenantRegistration.Create(...)` is the only correct factory for a
  registration; setters mutate, the constructor stays internal.

These rules exist because the entity types own their invariants;
side-stepping the factory leaves an entity in an invalid state that no
other call enforces.
