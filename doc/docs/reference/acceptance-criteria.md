...
- **AC-005** The first platform admin MUST be created by the
  `bootstrap-admin` CLI command, MUST receive a CSPRNG-generated password
  printed to stdout exactly once, and MUST be forced through
  `/change-password` on first authenticated request. EF Core migrations
  MUST NOT INSERT into `AspNetUsers`.
- **AC-006** The `bootstrap-admin` CLI command MUST refuse to run when any
  user already exists in the platform database.
...
The baseline is the English-first principle of AD-0015 and the
strategy in [`../explanation/concepts/internationalization.md`](../explanation/concepts/internationalization.md).

- **AC-117** Every source-code identifier under `/src/` and `/tests/`
  MUST be ASCII-only English (PascalCase, camelCase, or the naming
  rules in `/.editorconfig`); analyzers reject non-ASCII identifiers.
- **AC-118** Every database column, table, enum member, constraint,
  and sequence name MUST be English. The schema dump MUST contain no
  non-English identifier.
- **AC-119** Every user-facing string in a Razor component MUST be
  routed through `IStringLocalizer<T>`. A literal Turkish string in
  Razor markup is a build-time error (analyzer rule).
- **AC-120** A missing key in a non-English `*.resx` MUST fall through
  to the English neutral file at runtime. CI MUST fail the build if
  the English neutral file is missing a key referenced from code.
- **AC-121** Tenant `LanguageCode` MUST be one of the supported
  language codes in [`../explanation/concepts/internationalization.md`](../explanation/concepts/internationalization.md);
  an unsupported code rejects the `tenant.create` job with code
  `tenant.create.unsupported_language`.

## Data Protection

The baseline is the classification, retention, and rights handling in
[`../explanation/concepts/data-protection.md`](../explanation/concepts/data-protection.md).

- **AC-122** Every database column that stores `Sensitive` or
  `Restricted` data MUST carry a schema comment naming the data class.
  CI MUST fail the build if any such column has no comment.
- **AC-123** Every retention sweep job MUST write an audit-log entry
  summarising the entities deleted; a sweep that deletes nothing
  still writes an entry recording the run.
- **AC-124** A retention sweep MUST never delete an audit-log entry
  about itself or any audit-log entry younger than the audit retention
  window.
- **AC-125** Personal-data fields MUST NOT appear in application log
  output. CI's log-redaction analyzer MUST flag any structured-log
  property whose name matches the redaction list (`email`,
  `password*`, `token*`, `secret*`, `payment*`).
- **AC-126** A breach detection MUST trigger the procedure in
  [`../explanation/concepts/data-protection.md`](../explanation/concepts/data-protection.md#breach-notification);
  the affected tenant(s) MUST be notified within 24 hours of
  detection.

## Recovery

The baseline is the RTO/RPO targets in
[`./architecture/slos.md`](./architecture/slos.md#recovery-objectives).

- **AC-127** A backup MUST be encrypted at rest before it leaves the
  source host. Verification: a hex sample of any committed dump file
  MUST match the `age` envelope header.
- **AC-128** Off-site backup credentials MUST be append-only from the
  source host's perspective; a delete or overwrite attempt from the
  source MUST fail.
...
The baseline is the four-tier taxonomy in
[`../explanation/concepts/test-taxonomy.md`](../explanation/concepts/test-taxonomy.md).

- **AC-131** Every shipped capability MUST have at least one test in
  the appropriate tier. Capability matrix transition to `Implemented`
  MUST cite the test reference.
- **AC-132** A test marked `[Skip]` MUST NOT land on `main`. CI MUST
  reject any PR introducing a skipped test.
- **AC-133** No test in the `Unit` tier MAY touch the file system,
  the network, the system clock, or a database. Violations are caught
  by a static-analysis rule.
- **AC-134** The smoke suite MUST run against the freshly deployed
  environment after every deploy and MUST complete within 60 seconds
