...
| Tenant customer surfaces (Static SSR) | In progress | `/menu` and `/order/{id}` implemented; `/g/{token}` join flow pending. Render-mode annotation tracked under TD-0016. |
...
| Customer session with server-side cart | In progress | `customer_sessions`, `customer_access_tickets`, `customer_session_cart_items`, `qr_tokens` schema present; submit-order path implemented; **device-binding cookie** persisted on `CustomerAccessTicket.DeviceCookieValue` and verified at submit (PR #11, TD-0017 steps 1–3); **idempotency key** persisted on `Order` with unique index over `(SessionId, IdempotencyKey)` (PR #12, TD-0018 steps 1–2). Integration tests for both gates pending — see TD-0017 step 4 and TD-0018 step 3. |
...
| Test taxonomy (Unit / Integration / E2E / Smoke) via xUnit Traits | In progress | xUnit `[Trait("Category", T)]` convention adopted as tier discriminator (PR #13, TD-0010 steps 1–3); `tests/E2E.Tests/` added to `TabFlow.sln` and exposed via `extern alias` over both hosts; PR workflow splits Unit fast-path (no DB) from Integration step (PostgreSQL service container). Static-analysis rule for `Unit` tier (Roslyn analyzer flagging Npgsql/HttpClient/File/`DateTime.Now` imports), hermetic transactional fixture for the `Integration` tier, and Playwright bootstrap for E2E remain open under TD-0010 steps 4–6. |
...
| English-first lint enforcement | In progress | First-party Roslyn analyser `TabFlow.Analyzers.EnglishFirstIdentifierAnalyzer` (rule `TF0001`) ships from `/tools/analyzers/TabFlow.Analyzers/` (PR #14, TD-0009 steps 1–3); flags any non-ASCII character in declared identifiers (`NamedType`, `Method`, `Property`, `Field`, `Event`, `Parameter`); compiler-generated names (`<` / `$` prefix) skipped. `TreatWarningsAsErrors=true` promotes to a build break. xUnit regression suite (`Microsoft.CodeAnalysis.Testing`) and `AnalyzerReleases.{Shipped,Unshipped}.md` per RS2008 remain open under TD-0009 steps 4–5. |
