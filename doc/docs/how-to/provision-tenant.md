...
This guide presumes the platform itself is already running. If the
platform has not been brought up yet, run
[`./bootstrap-platform.md`](./bootstrap-platform.md) first.
...
11. Seed tenant defaults:
    ...
    - Default tenant admin identity reference
12. Verify tenant runtime health.
13. Mark the tenant `active`.

## Default Admin Baseline

When a tenant database is initialized:

- The initial admin email defaults to a tenant-scoped address derived from
  the platform domain template, or the optional admin email input.
- No usable default password is generated.
- Provisioning issues a one-time activation flow instead of a shared
  bootstrap secret.
- MFA readiness is part of the first privileged sign-in path.
...
- Seeded defaults are present:
  - Starter tables `000` and `999`
  - Starter catalog baseline
  - Default tenant admin activation path
...
2. Complete the admin activation or invitation flow.
3. Sign in at `https://<tenant-domain>/login`.
