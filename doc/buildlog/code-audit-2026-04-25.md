...
| H-3 (test projects by project, not tier) | Closed in source (partial) | PR #13: adopted `[Trait("Category", T)]` convention with values `Unit`, `Integration`, `E2E`, `Smoke`; classified existing tests; added `E2E.Tests` to the solution (with extern alias fix for the dual-Program CS0433 collision); split the PR workflow's `dotnet test` step into Unit (no DB) and Integration (PostgreSQL) tiers. The static-analysis rule that prevents Unit-tier tests from importing `Npgsql` / `HttpClient` / `System.IO.File` / `DateTime.Now` (TD-0010 step 4) and the hermetic per-test transactional fixture (step 5) and the E2E workflow (step 6) remain open. | 2026-04-25 |
