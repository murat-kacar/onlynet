...
The request MUST carry an `Idempotency-Key` header with a unique token
(UUIDv4 recommended) generated by the client for this submission
attempt. Replays of the same key within the session are acknowledged
with the original result; a new key that references a cart already
converted into an order is rejected with `order_duplicate`.
...
- Verifies and consumes the fresh QR checkout proof from the request
  body.
- Checks `Idempotency-Key` against the session; returns the prior
